...
void rrc::init(uint32_t               identifier,
...
void rrc::slice_configured(int slice_id, uint8 addr[]){
  pdcp->slice_configured(slice_id, addr);
}
...
void rrc::write_pdu_bcch_bch(byte_buffer_t *pdu)
{
  ...
  //pool->deallocate_shared_element(pdu);
	pool->deallocate(pdu);
...
void rrc::write_pdu_pcch(byte_buffer_t *pdu)
{
  if (pdu->N_bytes > 0 && pdu->N_bytes < SRSLTE_MAX_BUFFER_SIZE_BITS) {
    ...
    srslte_bit_unpack_vector(pdu->msg, bit_buf.msg, pdu->N_bytes*8);
    bit_buf.N_bits = pdu->N_bytes*8;
    rrc_log->info("RRC PASO A POR AQUI -- o en pcch\n");
    pool->deallocate(pdu);
...
void rrc::set_state(int new_state){
  switch(new_state)
  {
    case RRC_STATE_IDLE:
      state = RRC_STATE_IDLE;
    ...
    case RRC_STATE_SIB2_SEARCH:
      state = RRC_STATE_SIB2_SEARCH;
      break;
    case RRC_STATE_WAIT_FOR_CON_SETUP:
      state = RRC_STATE_WAIT_FOR_CON_SETUP;
      break;
    case RRC_STATE_COMPLETING_SETUP:
    ...
    case RRC_STATE_RRC_CONNECTED:
      ...
      break;
    case RRC_STATE_N_ITEMS:
      state = RRC_STATE_N_ITEMS;
      break;
    default:
      break;
	}
...
void rrc::set_sib1(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_1_STRUCT sib1_){
	memcpy(&sib1, &sib1_, sizeof(LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_1_STRUCT));
}

LIBLTE_RRC_SYS_INFO_BLOCK_TYPE_1_STRUCT rrc::get_sib1(){
	return sib1;
}
...
void rrc::send_con_request()
{
  ...
  if(nas->get_s_tmsi(&s_tmsi)) {
    ...
    ul_ccch_msg.msg.rrc_con_req.ue_id.random = 1000 + slice_identifier;
  }
  ...
  mac->set_contention_id(slice_identifier, uecri);

  rrc_log->info("Sending RRC Connection Request on SRB0\n");

  state = RRC_STATE_WAIT_FOR_CON_SETUP;
  //pdcp->write_sdu(slice_identifier, uecri, RB_ID_SRB0, pdcp_buf);
  pdcp->write_sdu_uecri(slice_identifier, uecri, RB_ID_SRB0, pdcp_buf);
}
...
void rrc::send_con_restablish_complete()
{
  ...
  pdcp->write_sdu(slice_identifier, RB_ID_SRB1, pdcp_buf);
}
...
void rrc::send_con_setup_complete(byte_buffer_t *nas_msg)
{
  ...
  ul_dcch_msg.msg.rrc_con_setup_complete.selected_plmn_id = slice_identifier+1;
  ...
  // Sinchronization between slices
  if (slice_identifier == 1){
		sleep(0.5);
  }
...
void rrc::send_ul_info_transfer(uint32_t lcid, byte_buffer_t *sdu)
{
  ...
  pdcp->write_sdu(slice_identifier, lcid, pdu);
}
...
void rrc::parse_dl_ccch(byte_buffer_t *pdu)
{
  ...
  byte_buffer_t *pdu_copy = new byte_buffer_t();
	if (slice_identifier == 0){
  	//byte_buffer_t *pdu_copy = new byte_buffer_t();
  	...
  	pdu_copy->N_bytes = pdu->N_bytes;
  	memcpy(pdu_copy->buffer, pdu->buffer, sizeof(pdu->buffer));
  	//memcpy(pdu_copy->msg,    msg_ptr,    (pdu->N_bytes*8)-(msg_ptr-pdu->msg));
		memcpy(pdu_copy->msg,    pdu->msg,    (pdu->N_bytes));
	}
  ...
  // If net_id == rrc.associated_slice --> handle connection
...
void rrc::parse_dl_dcch(uint32_t lcid, byte_buffer_t *pdu)
{
  ...
  case LIBLTE_RRC_DL_DCCH_MSG_TYPE_DL_INFO_TRANSFER:
    ...
    nas->write_pdu(lcid, pdu, this->slice_identifier);
    break;
  case LIBLTE_RRC_DL_DCCH_MSG_TYPE_SECURITY_MODE_COMMAND:
    ...
    pdcp->config_security(slice_identifier, lcid, k_rrc_enc, k_rrc_int, cipher_algo, integ_algo);
...
void rrc::handle_rrc_con_reconfig(uint32_t lcid, LIBLTE_RRC_CONNECTION_RECONFIGURATION_STRUCT *reconfig, byte_buffer_t *pdu)
{
  ...
  for(i=0;i<reconfig->N_ded_info_nas;i++)
  {
    ...
    nas->write_pdu(lcid, nas_sdu, this->slice_identifier);
  }
}
...
{
  // Setup PDCP
  pdcp->add_bearer(slice_identifier, srb_cnfg->srb_id);
  if(RB_ID_SRB2 == srb_cnfg->srb_id)
    pdcp->config_security(slice_identifier, srb_cnfg->srb_id, k_rrc_enc, k_rrc_int, cipher_algo, integ_algo);
...
void rrc::add_drb(LIBLTE_RRC_DRB_TO_ADD_MOD_STRUCT *drb_cnfg)
{
  ...
  // Setup PDCP
  pdcp->add_bearer(slice_identifier, lcid, &drb_cnfg->pdcp_cnfg);
