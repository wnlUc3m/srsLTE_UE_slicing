...
void nas::write_pdu(uint32_t lcid, byte_buffer_t *pdu, int slice_id)
{
  uint8 pd;
  uint8 msg_type;
  ...
  case LIBLTE_MME_MSG_TYPE_ATTACH_ACCEPT:
      parse_attach_accept(lcid, pdu, slice_id);
      break;
...
void nas::parse_attach_accept(uint32_t lcid, byte_buffer_t *pdu, int slice_id)
{
  ...
  if(attach_accept.eps_attach_result == LIBLTE_MME_EPS_ATTACH_RESULT_EPS_ONLY)
  {
    ...
    if(LIBLTE_MME_PDN_TYPE_IPV4 == act_def_eps_bearer_context_req.pdn_addr.pdn_type)
    {
      ...
      // Setup GW
      char *err_str = NULL;
      if(gw->setup_if_addr(ip_addr, err_str, slice_id))
      {
...
//    act_def_eps_bearer_context_req.connectivity_type_present
    ...
    rrc->write_sdu(lcid, pdu);
		//rrc->slice_configured(slice_id);
		uint8 ip_addr[12];
		memcpy(ip_addr, act_def_eps_bearer_context_req.pdn_addr.addr, sizeof(act_def_eps_bearer_context_req.pdn_addr.addr));
    rrc->slice_configured(slice_id, ip_addr);
  }
  else
  {
