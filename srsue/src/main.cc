...
void parse_args(all_args_t *args, int argc, char* argv[]) {
    ...
    common.add_options()
        ...
        // Second virtual UE
        ("usim.algo_2",         bpo::value<string>(&args->usim_2.algo),    "USIM2 authentication algorithm")
        ("usim.op_2",           bpo::value<string>(&args->usim_2.op),      "USIM2 operator variant")
        ("usim.amf_2",          bpo::value<string>(&args->usim_2.amf),     "USIM2 authentication management field")
        ("usim.imsi_2",         bpo::value<string>(&args->usim_2.imsi),    "USIM2 IMSI")
        ("usim.imei_2",         bpo::value<string>(&args->usim_2.imei),    "USIM2 IMEI")
        ("usim.k_2",            bpo::value<string>(&args->usim_2.k),       "USIM2 K")        
...
int main(int argc, char *argv[])
{
  ...
  while(running) {
    //cout << "---  Software Radio Systems LTE UE  --- " << running << endl;
    if (ue->is_attached()) {
