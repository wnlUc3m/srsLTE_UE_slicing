...
bool mac::init(phy_interface_mac *phy,
               ...
               srslte::log *log_h_)
{
  ...
  ra_procedure.init (phy_h, rrc_h[0], log_h, &uernti, &config, &timers_db, &mux_unit, &demux_unit);
  sr_procedure.init (phy_h, rrc_h[0], log_h,          &config);
...
void mac::timeAlignmentTimerExpire() 
{
  ...
  //rrc_h->release_pucch_srs();
   // We have to notify all the slices
  for(int slice = 0; slice < NUM_SLICES; ++slice){
    rrc_h[slice]->release_pucch_srs();
  }
  dl_harq.reset();
  ul_harq.reset();
}
...
void mac::set_contention_id(int slice_identifier, uint64_t uecri)
{
