  ...
  while(pdu_msg->next()) {
    if (pdu_msg->get()->is_sdu()) {
      ...
      if (route_pdu) {
        ...
        rlc->write_pdu(pdu_msg->get()->get_sdu_lcid(), slice_id, pdu_msg->get()->get_sdu_ptr(), pdu_msg->get()->get_payload_size());      
      }
    } else {
      ...
      else if((pdu_msg->get()->ce_type() == srslte::sch_subh::CON_RES_ID)&&(slice_id == 0)){
        uint8_t *contention_resolution_header = pdu_msg->get()->get_sdu_ptr();
        uint8_t payload_len = pdu_msg->get()->get_payload_size();
        for (uint8_t r = 0; r < payload_len; ++r){
          // Header is split in 8 bit chunks -- Conversion to decimal => chunk representation * 256^x, where x increases from 0 to payload_size form right to left
          //  Similar to hexadecimal to decimal but using 256 (8 bits representation) instead of 16 bits (4 bits representation)
          slice_id += contention_resolution_header[r] * pow(256, (payload_len-1)-r);
        }
      }
    }
  }      
