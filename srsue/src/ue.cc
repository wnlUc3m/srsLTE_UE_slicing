...
bool ue::init(all_args_t *args_)
{
  ...
  // Array creation //
  // ************** //
  srsue::rrc_interface_rlc    *rrc_rlc[2];
  srsue::rrc_interface_pdcp   *rrc_pdcp[2];
  srsue::gw_interface_pdcp    *gw_pdcp[2];
  for(int slice = 0; slice < NUM_SLICES; ++slice){
    rrc[slice] = new srsue::rrc();
    rrc_rlc[slice] = rrc[slice];
    rrc_pdcp[slice] = rrc[slice];
    nas[slice] = new srsue::nas();
    gw[slice] = new srslte::gw();
    gw_pdcp[slice] = gw[slice];
    usim[slice] = new srsue::usim();
  }
  ...
  // ************************** //
  // Init NOT duplicated layers //
  // ************************** //
  mac.init(&phy, &rlc, rrc, &mac_log);
  rlc.init(&pdcp, rrc_rlc, this, &rlc_log, &mac);
  pdcp.init(&rlc, rrc_pdcp, gw_pdcp, &pdcp_log, SECURITY_DIRECTION_UPLINK);


  // ********************** //
  // Init duplicated layers //
  // ********************** //
  rrc[0]->init(0, &phy, &mac, &rlc, &pdcp, nas[0], usim[0], &mac, &rrc_log);
  rrc[0]->set_ue_category(args->expert.ue_cateogry);
  rrc[1]->init(1, &phy, &mac, &rlc, &pdcp, nas[1], usim[1], &mac, &rrc_log);
  rrc[1]->set_ue_category(args->expert.ue_cateogry);

  nas[0]->init(usim[0], rrc[0], gw[0], &nas_log);
  nas[1]->init(usim[1], rrc[1], gw[1], &nas_log);

  gw[0]->init(&pdcp, rrc[0], this, &gw_log);
  gw[1]->init(&pdcp, rrc[1], this, &gw_log);

  usim[0]->init(&args->usim, &usim_log);
  usim[1]->init(&args->usim_2, &usim_log);
...
void ue::test_con_restablishment() {
  rrc[0]->test_con_restablishment();
  rrc[1]->test_con_restablishment();
}
...
{
  // gines
  if ((EMM_STATE_REGISTERED == nas[0]->get_state()) && (EMM_STATE_REGISTERED == nas[1]->get_state())){
    return true;
  }
  else{
    return false;
...
bool ue::get_metrics(ue_metrics_t &m)
{
  ...
  if((EMM_STATE_REGISTERED == nas[0]->get_state()) && (EMM_STATE_REGISTERED == nas[1]->get_state())) {
    if((RRC_STATE_RRC_CONNECTED == rrc[0]->get_state()) && (RRC_STATE_RRC_CONNECTED == rrc[1]->get_state())) {
      phy.get_metrics(m.phy);
      mac.get_metrics(m.mac);
      rlc.get_metrics(m.rlc);
      //gw.get_metrics(m.gw);
      // Gines -- 
      gw[0]->get_metrics(m.gw);
      gw[1]->get_metrics(m.gw);
      return true;
    }
  }
  return false;
