...
/*******************************************************************************
  NAS interface
*******************************************************************************/
error_t gw::setup_if_addr(uint32_t ip_addr, char *err_str, int slice_id)
{
  if(!if_up)
  {
      if(init_if(err_str, slice_id))
      {
  ...
  // Set default routes for each slice
  int pid = fork();
  if (pid == 0){
    if (slice_id == 0){
      if (system("sudo ip r a 192.168.100.0/24 via 172.16.0.1 dev tun_srsue_0") == -1){
        gw_log->console("GW -- Error setting default route Slice 1");
      }
    }
    else{
      if (system("sudo ip r a 192.168.200.0/24 via 172.20.0.1 dev tun_srsue_1") == -1){
        gw_log->console("GW -- Error setting default route Slice 2");
      }
    }
  }
...
error_t gw::init_if(char *err_str, int slice_id)
{
    if(if_up)
    {
    ...
    //dev[IFNAMSIZ] = itoa(slice_id);
    sprintf(dev, "tun_srsue_%i", slice_id);
      ...
      if(N_bytes > 0)
      {
        ...
        if (ip_pkt->version == 4) {
          ...
          if(ntohs(ip_pkt->tot_len) == pdu->N_bytes)
          {
            ...
            pdcp->write_sdu(rrc->get_identifier(), RB_ID_DRB1, pdu);
