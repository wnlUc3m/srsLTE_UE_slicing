...
void rlc::write_pdu(uint32_t lcid, uint64_t slice_id, uint8_t *payload, uint32_t nof_bytes)
{
  if(valid_lcid(lcid)) {
    dl_tput_bytes[lcid] += nof_bytes;
    rlc_array[lcid].write_pdu(slice_id, payload, nof_bytes);
  }
}
