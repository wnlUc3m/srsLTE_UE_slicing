...
{
  log->info_hex(sdu->msg, sdu->N_bytes, "RLC_AM %s Tx SDU", rb_id_text[lcid]);
...
void rlc_am::write_pdu(uint64_t slice_id, uint8_t *payload, uint32_t nof_bytes)
{
  if(nof_bytes < 1)
    return;
  pthread_mutex_lock(&mutex);
  ...
  if(rlc_am_is_control_pdu(payload)) {
    ...
    if(header.rf) {
      handle_data_pdu_segment(slice_id, payload, nof_bytes, header);
    }else{
      handle_data_pdu(payload, nof_bytes, header, slice_id);
    }
  }
  pthread_mutex_unlock(&mutex);
}
...
void rlc_am::handle_data_pdu(uint8_t *payload, uint32_t nof_bytes, rlc_amd_pdu_header_t header, uint64_t slice_id)
{
...
void rlc_am::handle_data_pdu_segment(uint64_t slice_id, uint8_t *payload, uint32_t nof_bytes, rlc_amd_pdu_header_t header)
{
  ...
  if(rx_segments.end() != it) {
    ...
    if(add_segment_and_check(&it->second, &segment, slice_id)) {
      std::list<rlc_amd_rx_pdu_t>::iterator segit;
...
void rlc_am::reassemble_rx_sdus(uint64_t slice_id)
{
  if(!rx_sdu) {
  ...
  while(rx_window.end() != rx_window.find(vr_r))
  {
    ...
    for(uint32_t i=0; i<rx_window[vr_r].header.N_li; i++)
    {
      ...
      pdcp->write_pdu(lcid, slice_id, rx_sdu);
      rx_sdu = pool_allocate;
      if (!rx_sdu) {
...
bool rlc_am::add_segment_and_check(rlc_amd_rx_pdu_segments_t *pdu, rlc_amd_rx_pdu_t *segment, uint64_t slice_id)
{
  // Ordered insert
  ...
  handle_data_pdu(full_pdu->msg, full_pdu->N_bytes, header, slice_id);
  return true;
}
