...
void rlc_um::write_pdu(uint64_t slice_id, uint8_t *payload, uint32_t nof_bytes)
{
  pthread_mutex_lock(&mutex);
  handle_data_pdu(payload, nof_bytes, slice_id);
  pthread_mutex_unlock(&mutex);
}
...
void rlc_um::timer_expired(uint32_t timeout_id)
{
  if(reordering_timeout_id == timeout_id)
  {
    ...
    while(RX_MOD_BASE(vr_ur) < RX_MOD_BASE(vr_ux))
    {
      ...
      // TODO: gestionar de forma correcta ambas slices
      reassemble_rx_sdus(0);
...
void rlc_um::handle_data_pdu(uint8_t *payload, uint32_t nof_bytes, uint64_t slice_id)
{
  std::map<uint32_t, rlc_umd_pdu_t>::iterator it;
  rlc_umd_pdu_header_t header;
...
void rlc_um::reassemble_rx_sdus(uint64_t slice_id)
{
  if(!rx_sdu)
