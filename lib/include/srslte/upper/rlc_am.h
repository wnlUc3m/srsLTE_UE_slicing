...
public:
  ...
  void     write_pdu(uint64_t slice_id, uint8_t *payload, uint32_t nof_bytes);
...
private:
  ...
  void handle_data_pdu(uint8_t *payload, uint32_t nof_bytes, rlc_amd_pdu_header_t header, uint64_t slice_id);
  void handle_data_pdu_segment(uint64_t slice_id, uint8_t *payload, uint32_t nof_bytes, rlc_amd_pdu_header_t header);
  void handle_control_pdu(uint8_t *payload, uint32_t nof_bytes);

  void reassemble_rx_sdus(uint64_t slice_id);

  bool inside_tx_window(uint16_t sn);
  bool inside_rx_window(uint16_t sn);
  void debug_state();

  bool add_segment_and_check(rlc_amd_rx_pdu_segments_t *pdu, rlc_amd_rx_pdu_t *segment, uint64_t slice_id);
