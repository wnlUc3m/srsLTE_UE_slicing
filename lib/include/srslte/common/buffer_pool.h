...
public:
  ...
  bool deallocate_shared_element(buffer_t *b)
  {
		printf("BUFFER_POOL -- READ = %i\n",READ);
    bool ret = false; 
    pthread_mutex_lock(&mutex);
    typename std::vector<buffer_t*>::iterator elem = std::find(used.begin(), used.end(), b);
    if (elem != used.end()) {
      if (READ == 0){
        ...
        ret = true;
			...
			printf("BUFFER_POOL -- salir READ = %i\n",READ);
      //used.erase(elem); 
      //available.push(b);
      //ret = true; 
    } else {
...
public: 
  ...
  void deallocate_shared_element(byte_buffer_t *b){
    pool->deallocate_shared_element(b);
  }
